...
Load-bearing invariant — **do not weaken it**: the profile *library* is an operator-curated file (`RS3005P_PROFILE`, or `--profile`); **no MCP tool may create or modify a profile's limits**, so an agent can never fabricate a wider envelope. A tool *may* **select** which curated device in that library is active (`select_device`, or `connect(device=...)`, or the `RS3005P_DEVICE` default) — switching to a profile whose envelope is wider in any dimension requires `confirm_widen=True`, and every switch forces the supply to the new profile's safe baseline. The library file is re-read on each connect/select (hot reload), so editing `devices.json` needs no re-registration. Enforcement (V/I/power ceilings, output gating, slew limiting, connect/recall/select safe-baseline reset) lives in `device.py`; pure checks live in `safety.py`. Voltage `nominal±tolerance` sets the **upper ceiling** + nominal target with the floor at 0 V (ramping up / switching off must always be allowed); a hard floor needs explicit `{min, max}`. The residual risk runtime selection accepts: a wrong-profile-for-attached-hardware mismatch is the operator's responsibility (it can't be detected in software) — the confirm-to-widen gate and forced baseline bound the blast radius. See `docs/safety.md`.
